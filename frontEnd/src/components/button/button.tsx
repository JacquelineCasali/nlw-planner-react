import { ComponentProps, ReactNode } from "react"
import {tv, VariantProps} from 'tailwind-variants'


const buttonVariants = tv({
    
    //  propriedade iguais em todos os butoes
    base: 'rounded-lg px-5   font-medium flex items-center justify-center gap-2',


  //  propriedade diferentes  em todos os butoes
  variants:{
    variant:{
        green:'bg-lime-300 text-lime-950 hover:bg-lime-400',
        cinza:'bg-zinc-800 text-zinc-200 hover:bg-zinc-700'
    },
    size:{
        default:'py-2',
        full:'w-full h-11'
    }
  },
//   cor nao informada colca o verde por padrao 
defaultVariants:{
    variant:'green',
    size:'default'
}

})
//conteudo para um componente usa o children
// ReactNode passa qualquer coisa 
interface ButtonProps extends ComponentProps<'button'>,VariantProps<typeof buttonVariants> {
    children:ReactNode
}

export default function Button({children,variant,size,...props }:ButtonProps) {
  return (
    // {...props} todas as propriedades do botao
    <button {...props}
     className={buttonVariants({variant,size})}
  >
{children}

  </button>
  )
}

